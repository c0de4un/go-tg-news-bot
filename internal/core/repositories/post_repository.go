...
func (pr *PostRepository) SetState(id int64, status int64) error {
...
func (pr *PostRepository) GetPostById(id int64) (*models.PostModel, error) {
	dbm, _ := database.GetDBManager()
	if dbm == nil {
		fmt.Println("PostRepository.GetPostById: dbm is nil, maybe app is terminating")
		return nil, fmt.Errorf("PostRepository.GetPostById: dbm is nil, maybe app is terminating")
	}
...
func (pr *PostRepository) buildFromRow(row *sql.Row) (*models.PostModel, error) {
	post := &models.PostModel{}
	...
	if err != nil {
		fmt.Printf("PostRepository.buildFromRow: %s", err)
		return nil, err
	}
